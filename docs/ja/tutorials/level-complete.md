...
このサンプルプロジェクトでは、レベルをクリアしたときに使える、スコアをカウントアップして表示する演出を紹介します。プロジェクトは[エディターから開く](/manuals/project-setup/)か、[GitHub からダウンロード](https://github.com/defold/sample-levelcomplete)できます。合計スコアをカウントアップし、異なる基準スコアに達するたびに、3つの星を1つずつ表示します。また、このサンプルではリロード（reload）機能を使い、数値の調整結果をすばやく確認できるようにしています。
