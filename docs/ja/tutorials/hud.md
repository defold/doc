...
このサンプルプロジェクトは、[エディターから開く](/manuals/project-setup/)か、[GitHub からダウンロードする](https://github.com/defold/sample-hud)ことができます。ここでは、HUD（ゲーム中の情報表示）でスコアを加算するときのエフェクトを紹介します。スコアは画面上のランダムな位置に表示され、プレイヤーがさまざまな位置でスコアを獲得するゲームを再現します。
