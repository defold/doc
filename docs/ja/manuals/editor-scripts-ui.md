...
このマニュアルでは、Lua で記述したエディタースクリプト（editor script）を使用して、エディターに操作可能なダイアログを作成し、リソースを開く方法を説明します。エディタースクリプトを使い始めるには、[エディタースクリプトのマニュアル](/manuals/editor-scripts)を参照してください。エディター API の完全なリファレンスは[こちら](/ref/editor-lua/)にあります。
