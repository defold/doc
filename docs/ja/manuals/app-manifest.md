...
有効（`true`）にすると、プロジェクトで True Type Fonts（`.ttf`）を使用する場合に、SDF 型フォントを実行時に生成できるようになります。詳細は[フォントのマニュアル](https://defold.com/manuals/font/#enabling-runtime-fonts)を参照してください。
