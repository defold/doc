...
Defold 1.13.2 以降では、[モデルコンポーネント](/manuals/model/#model-properties)でインポートしたシーンから名前付きメッシュを1つ選択できます。*Mesh* フィールドを空にすると、シーン全体を使い、上記のトランスフォームを保持します。メッシュを選択すると、glTF ノードのトランスフォームを適用せずに、そのローカルジオメトリを使います。そのため、モデルコンポーネントまたはゲームオブジェクトのトランスフォームで配置してください。
