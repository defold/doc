...
![Animation set](images/icons/animationset.png){.left} An animation set resource contains a list of glTF files or other .animationset files from where to read animations. Adding one .animationset files to another is handy if you share partial sets of animations between several models. See the [model animation manual](/manuals/model-animation/) for details.
