...
* Модели glTF (*.gltf* и *.glb*) могут использоваться [компонентом Model](/manuals/model) для отображения 3D-моделей с анимацией. Импортируйте все изображения текстур, используемые моделью, как отдельные ассеты и назначайте их в свойствах текстур материала компонента Model. См. [как импортировать и использовать 3D-модели](/manuals/importing-models).
