---
title: Сообщения о столкновениях в Defold
brief: Когда два объекта сталкиваются, движок широковещательно рассылает сообщения всем компонентам в обоих объектах.
---

# Сообщения о столкновениях

Когда два объекта сталкиваются, движок широковещательно рассылает сообщения всем компонентам в обоих объектах:

## Ответ о столкновении

Самое сообщение `"collision_response"` посылается на все объекты столкновения. У него имеются следующий набор полей:
...
`relative_velocity`
: относительная скорость объекта столкновения наблюдаемая от лица другого объекта (`vector3`).
...
Для игры или ряда применений, где вам нужно идеально отделить объекты, сообщение `"contact_point_response"` даёт всю необходимую для этого информацию. Однако, заметьте, что для любой пары столкновения, каждый кадр могут присылаться несколько сообщений `"contact_point_response"`, в зависимости от природы столкновения. Смотрите [Разрешение столкновений для более подробной информации](/manuals/physics-resolving-collisions).
