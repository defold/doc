...
Можно включить одну из архитектур или обе. Если включены обе, загрузчик выбирает `wasm_pthread-web`, когда браузер и среда хостинга её поддерживают, и в противном случае переходит на `wasm-web`. Канонические имена целей приведены в [руководстве по Bob](/manuals/bob/#usage).
...
Выберите запрашиваемый графический контекст с помощью [`graphics.webgl_version_hint`](/manuals/project-settings/#webgl-version-hint). По умолчанию используется WebGL 2; запросите WebGL 1, чтобы тестировать или использовать этот контекст в браузерах, поддерживающих обе версии.
...
Отключение **Verify Downloaded File Size** не отключает проверку SHA-1, если она включена в бандл. См. [настройки проекта для HTML5](/manuals/project-settings/#verify-downloaded-file-size).
