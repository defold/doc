...
| *rotation* | Локальное вращение игрового объекта, выраженное в виде `quaternion` | `quaternion` | `get+set`{.mark} |
| *euler*    | Локальное вращение игрового объекта в углах Эйлера | `vector3` | `get+set`{.mark} |
| *scale*    | Локальное неоднородное масштабирование игрового объекта, выраженное в виде вектора, каждый компонент которого содержит множитель по соответствующей оси. Чтобы удвоить размер по осям X и Y, не изменяя Z, используйте `vmath.vector3(2.0, 2.0, 1.0)`. | `vector3` | `get+set`{.mark} |
| *scale.xy* | Локальное неоднородное масштабирование игрового объекта по осям X и Y. Используйте это свойство или `go.set_scale_xy()`, если масштабирование по Z не требуется. | `vector3` | `get+set`{.mark} |
...
| *texture0*--*texture15* | Хэши путей текстур модели. | `hash` | `get+set`{.mark}|
...
| *material* | Материал, используемый моделью. | `hash` | `get+set`{.mark} |

*СВОЙСТВА КОМПОНЕНТА LABEL*

| Свойство | Описание                    | Тип         |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Текстовое содержимое метки. Доступно начиная с Defold 1.13.2. | `string` | `get+set`{.mark} |
...
| *material* | Материал, используемый меткой. | `hash` | `get+set`{.mark} |
| *font* | Шрифт, используемый меткой. | `hash` | `get+set`{.mark} |
