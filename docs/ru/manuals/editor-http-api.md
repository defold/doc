...
Редактор — графическое настольное приложение. Запускайте его в интерактивном пользовательском сеансе с доступом к дисплею. Если графический сеанс недоступен, например в headless-CI, либо для создания автономных бандлов, используйте [Bob](/manuals/bob). Открытый редактор также поддерживает автоматизацию только компиляции через `/command/compile`.
...
Завершение сборки не означает, что игра уже загрузилась в браузере. Дождитесь готовности canvas и приложения перед отправкой ввода или проверкой игрового процесса. Подробнее см. в разделе [Браузерные тесты для HTML5](/manuals/automated-testing/#browser-tests-for-html5).
