...
: Кинематические объекты только регистрируют столкновения с другими физическими объектами, но физический движок не производит никаких автоматических симуляций. Решение о разрешении столкновений, либо их игнорировании, остается за вами ([узнайте больше](/manuals/physics-resolving-collisions)). Кинематические объекты весьма хороши для игрового персонажа или для объектов, управляемых скриптами, которые требуют тонкого контроля над физическими реакциями, как например игровой персонаж.

Триггеры
: Триггеры --- это объекты, которые регистрируют простые столкновения. Триггеры --- это облегченные версии объектов столкновения. Они похожи на [рейкасты](/manuals/physics-ray-casts) в том смысле, что считывают состояние физического мира, а не взаимодействуют с ним. Они хорошо подходят для объектов, которым нужно только зарегистрировать попадание (например, пуле), а также для игровой логики, где нужно запускать особые действия, когда объект достигает определенной точки. Триггеры вычислительно дешевле, чем кинематические объекты, и по возможности их стоит предпочитать.
...
Компонент столкновения может содержать несколько встроенных форм, включая выпуклые оболочки и треугольные сетки в 3D-физике, либо использовать ресурс тайловой карты или выпуклой формы. За подробностями о различных формах и о том, как их добавлять к объекту столкновения, обращайтесь к [руководству по формам столкновений](/manuals/physics-shapes).
...
Collision Shape
: Ресурс тайловой карты или `.convexshape`. Чтобы использовать сетку glTF или GLB, добавьте в компонент форму Hull или Mesh и задайте её свойства *Scene* и *Mesh*. За подробностями обращайтесь к [руководству по формам столкновения](/manuals/physics-shapes).
