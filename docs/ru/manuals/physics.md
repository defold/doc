...
Defold включает [Box2D](https://box2d.org/) для симуляции 2D-физики и Bullet для 3D-физики. Настройка [Physics 2D в манифесте приложения](/manuals/app-manifest/#physics-2d) позволяет выбрать **Box2D Version 3**, **Box2D (Legacy Defold version)** или **None**. По умолчанию используется прежняя реализация, а Box2D 3 включается явно. Смена реализации может изменить результаты симуляции и потребовать перенастройки зависящих от версии [параметров проекта Box2D](/manuals/project-settings/#box2d).
...
: Если вы испытываете проблемы с тем, что столкновения не обрабатываются или не детектируются, то убедитесь, что вам знаком весь материал из секции по [отладке физики в руководстве по отладке](/manuals/debugging-game-logic/#debugging-problems-with-physics).
