...
Создание бандла для Linux не требует специальной настройки и дополнительной конфигурации для конкретной платформы в [файле настроек проекта][project settings file](/manuals/project-settings/) *game.project*.
...
* По умолчанию релизные сборки не содержат [профайлер](/manuals/profiling). Чтобы включить поддержку профайлера как в отладочных, так и в релизных сборках, установите для параметра **Profiler** значение **Always** в [манифесте приложения](/manuals/app-manifest/#profiler).
