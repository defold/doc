...
* `**` соответствует любому количеству символов, включая `/`
...
Это исключит папку `tiled` в каждой непосредственной подпапке `/levels` (например, `/levels/01/tiled`), все папки с именем `generated` внутри любой подпапки, а также файлы вроде `/assets/temp_01.png`.

Обратите внимание, что `**` не соответствует окружающим его косым чертам, поэтому `/levels/**` соответствует всему содержимому `/levels`, но не самой папке `/levels`, а `/**/generated` не соответствует `/generated` в корне проекта.
