...
![Model](images/icons/model.png){.left} С помощью компонента 3D-модели можно импортировать в игру glTF-сетку, скелет и анимационные ассеты. За подробностями обращайтесь к [руководству по моделям](/manuals/model/).
...
![Sound](images/icons/sound.png){.left} Звук --- это компонент, отвечающий за воспроизведение определённого звука. Defold поддерживает файлы WAV, Ogg Vorbis и Ogg Opus. Поддержку Opus необходимо включить в манифесте приложения. За подробностями обращайтесь к [руководству по звукам](/manuals/sound).
