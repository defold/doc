...
* GUI, эффекты частиц и тайловые карты — эти компоненты можно исключить по отдельности с помощью [переключателей компонентов в App Manifest](/manuals/app-manifest/#exclude-gui). Удалите ссылки на компоненты и вызовы API каждой исключаемой функции. Исключение эффектов частиц также исключает поддержку нод частиц в GUI-сценах.
* Форматированный текст — отключите [Use Rich Text](/manuals/app-manifest/#use-rich-text), если меткам и GUI-тексту нужен только обычный текст. Это исключает разбор текстовой разметки и стилевые эффекты, сохраняя обычную отрисовку текста.
...
Defold поддерживает Wave-файлы с 8- и 16-битным PCM. Ogg Vorbis и Ogg Opus используют собственные сжатые форматы, поэтому требование к разрядности PCM к ним не относится. Декодер Opus по умолчанию не включён; прежде чем использовать ресурсы `.opus`, включите **Include Sound Decoder: Opus** в [манифесте приложения](/manuals/app-manifest/#sound).
