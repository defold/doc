...
* GUI, 파티클 이펙트, 타일맵 - [App Manifest 컴포넌트 스위치](/manuals/app-manifest/#exclude-gui)로 각각 제외할 수 있습니다. 제외하는 기능의 컴포넌트 참조와 API 호출을 제거하세요. 파티클 이펙트를 제외하면 GUI 씬의 파티클 노드 지원도 제거됩니다.
* 리치 텍스트 - 라벨과 GUI 텍스트에 일반 텍스트만 필요하다면 [Use Rich Text](/manuals/app-manifest/#use-rich-text)를 비활성화합니다. 일반 텍스트 렌더링은 유지하면서 리치 텍스트 파싱과 스타일 효과를 제거합니다.
