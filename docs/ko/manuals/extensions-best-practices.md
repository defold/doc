...
Defold 소스는 각 컴파일러의 기본 C++ 버전으로 빌드됩니다. Defold 소스 자체는 대부분 C++98보다 높은 C++ 버전을 사용하지 않지만, 일부에서는 C++11을 사용합니다. 익스텐션을 빌드할 때 더 높은 버전을 사용할 수는 있지만, 더 높은 버전에는 ABI 변경이 함께 올 수 있습니다. 이로 인해 엔진에 포함된 익스텐션이나 [Asset Portal](/assets)의 익스텐션과 함께 하나의 익스텐션을 사용하는 것이 불가능해질 수 있습니다.
