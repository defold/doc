...
프로젝트가 Git을 사용한다면 이 뷰는 현재 커밋(`HEAD`)과 비교해 로컬에서 변경, 추가, 이름 변경 또는 삭제된 파일을 나열합니다. 원격 저장소와 동기화하려면 외부 Git 클라이언트나 커맨드 라인을 사용하세요. 자세한 내용은 [Version Control 매뉴얼](/manuals/version-control/)에서 확인할 수 있습니다. 이 뷰에서 일부 파일 관련 작업을 수행할 수 있습니다.
