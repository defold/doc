...
Defold 1.13.2부터 [Model 컴포넌트](/manuals/model/#model-properties)에서 임포트된 씬의 이름 있는 메쉬 하나를 선택할 수 있습니다. *Mesh* 필드를 비워 두면 전체 씬을 사용하고 위에서 설명한 트랜스폼을 보존합니다. 메쉬를 선택하면 glTF 노드 트랜스폼 없이 로컬 지오메트리를 사용하므로, 모델 컴포넌트나 게임 오브젝트의 트랜스폼으로 배치하세요.
