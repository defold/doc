...
#### Info.plist
지정하면 앱을 번들링할 때 내장 iOS 기본 메니페스트 대신 이 *`Info.plist`* 파일을 사용합니다. 내장 메니페스트에는 릴리스가 아닌 빌드에서 에디터 타겟을 검색하는 데 필요한 로컬 네트워크 및 Bonjour 항목이 들어 있습니다. 커스텀 메니페스트를 제공하면서 디바이스의 타겟 검색, 프로파일링, 핫 리로드 또는 로그 스트리밍이 필요하다면 [iOS 매뉴얼](/manuals/ios/#creating-an-ios-application-bundle)에 설명된 항목을 유지하세요.
...
App Manifest의 **Profiler** 설정은 프로파일러 코드를 디버그 빌드와 릴리스 빌드에 링크할지 제어합니다. 아래 설정은 선택한 빌드에 포함된 프로파일러 코드의 런타임 동작을 제어합니다. 자세한 내용은 [프로파일링 매뉴얼](/manuals/profiling/)을 참고하세요.
...
메인 프로젝트 또는 [네이티브 익스텐션](/manuals/extensions/)에 커스텀 설정을 정의할 수 있습니다. 메인 프로젝트의 커스텀 설정은 프로젝트 루트의 `game.properties` 파일에 정의해야 합니다. `ext.properties`라는 파일은 프로젝트와 가져온 라이브러리 종속성 어디에서나 발견되며, 이웃한 `ext.manifest`가 필요하지 않습니다. 발견된 모든 익스텐션 메타데이터를 병합한 뒤 루트의 `game.properties` 파일을 적용하며, 이 파일이 앞선 값을 override할 수 있습니다.
