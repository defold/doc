...
본 행렬 캐시는 `RGBA32F` 텍스쳐를 사용합니다. 활성 그래픽 어댑터가 이 텍스쳐 형식을 지원하지 않으면 Defold는 로컬 공간 메터리얼을 사용하는 애니메이션 Model 컴포넌트를 생성할 수 없습니다. 따라서 OpenGL ES 2.0과 WebGL 1.0에서는 어댑터의 부동 소수점 텍스쳐 확장 지원 여부에 따라 달라집니다. 이를 지원하지 않는 어댑터와 호환하려면 CPU 스키닝을 사용하고 모델 인스턴싱을 사용할 수 없는 커스텀 월드 공간 메터리얼을 사용하세요. 캐시 크기는 [Model 프로젝트 설정](/manuals/project-settings/#model)에서 조정할 수 있습니다.
