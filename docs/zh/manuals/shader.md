...
Defold 在将着色器交叉编译为 GLSL ES 时生成全局默认精度限定符。浮点值默认为 `mediump`，整数默认为 `highp`。可通过[项目设置](/manuals/project-settings/#shader)中的 **GLSL ES Default Precision Float**（`shader.glsl_es_default_precision_float`）和 **GLSL ES Default Precision Int**（`shader.glsl_es_default_precision_int`）更改；两者都接受 `mediump` 或 `highp`。
