...
* GUI、粒子效果和图块地图 - 可以通过 [App Manifest 组件开关](/manuals/app-manifest/#exclude-gui)分别排除这些组件。请移除所排除功能的组件引用和 API 调用。排除粒子效果还会移除对 GUI 场景中粒子节点的支持。
* 富文本 - 如果标签和 GUI 文本仅需要纯文本，请禁用 [Use Rich Text](/manuals/app-manifest/#use-rich-text)。这会移除富文本解析和样式效果，同时保留普通文本渲染。
