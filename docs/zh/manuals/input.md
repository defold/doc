...
无论哪种方案都必须将鼠标手点选的屏幕坐标转换成游戏对象的世界坐标. 实现思路如下:
  ...
  * 手动跟踪渲染脚本使用的视口和投射用以进行坐标转换. 详见 [摄像机教程的这个示例](/manuals/camera/#converting-mouse-to-world-coordinates).
  * 使用 [第三方摄像机解决方案](/manuals/camera/#third-party-camera-solutions) 里面的屏幕到世界坐标转换函数.
