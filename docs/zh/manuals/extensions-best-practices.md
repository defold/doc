...
```
    /root
        ...
        /bundleres                       -- 应该打包的资源（参见 game.project 和 [bundle_resources 设置]([physics scale setting](/manuals/project-settings/#project)）
            /<platform>
        game.project
...
Defold 源码是使用每个编译器的默认 C++ 版本构建的。Defold 源码本身在大多数地方使用的 C++ 版本不高于 C++98，但在一些地方使用 C++11。虽然可以使用更高版本来构建扩展，但更高版本可能带来 ABI 变化。这可能会使得无法在引擎或[资源门户](/assets)中将一个扩展与其他扩展一起使用。
