...
这就建立了一个 HTTP GET 请求发往 https://www.defold.com. 函数是异步的所以发送请求时不会卡住游戏. 一旦服务器有了回应, 就会自动调用回调函数. 回调函数里包含服务器响应的所有数据, 包括状态码和响应头.

同样可以建立 HTTP POST 请求来向服务器发送数据并指定请求头:
