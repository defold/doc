...
这将排除 `/levels` 每个直接子文件夹中的 `tiled` 文件夹（例如 `/levels/01/tiled`）、任意子文件夹下所有名为 `generated` 的文件夹，以及诸如 `/assets/temp_01.png` 之类的文件。

注意 `**` 不匹配其两侧的斜杠，因此 `/levels/**` 匹配 `/levels` 内的所有内容，但不匹配 `/levels` 文件夹本身，而 `/**/generated` 也不匹配项目根目录下的 `/generated`。
