...
默认情况下，Bob 在当前目录中寻找项目，并将从 `game.project` 可达的资源构建到 *build/default* 中。未被引用的资源不会编译。如果代码在运行时按路径加载原始文件，请通过[自定义资源项目设置](/manuals/project-settings/#custom-resources)将其包含进来；其他 Defold 资源需要有从某个构建根可达的引用。
