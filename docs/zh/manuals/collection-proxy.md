...
在代理既未加载也未正在加载时，调用 `collectionproxy.set_collection("#level_proxy", nil)` 可恢复编辑器中分配的集合。下载和挂载内容请参阅 [Live Update 脚本手册](/manuals/live-update-scripting/)；`collectionproxy.RESULT_*` 失败代码请参阅 API 参考。
